...
var pageHeight = pages[0].offsetHeight -100; //height of each page (sections)
...
//first not included, would be 0
function getPageScrollHeight() {
  pageHeight = pages[0].offsetHeight;
  let pageScrollHeight = [];
  for (var i = 1; i < pages.length; i++) {
    pageScrollHeight.push(pageHeight*i-100); //40px tolerance to change more rapid
  }
...
window.addEventListener('resize', () => {
  if (pageHeight != pages[0].offsetHeight) {  //if section size actually changed
...
window.addEventListener('scroll', () => {
  if (this.scrollY < pageScrollHeight[0]) {
    toggleNavActive(1);

  } else if (this.scrollY >= pageScrollHeight[0] && this.scrollY < pageScrollHeight[1]) {
    toggleNavActive(2);
  ...
  } else if (this.scrollY >= pageScrollHeight[4]) {
    toggleNavActive(6);
  }

  //toggle fisrt page header and other page header
  if (this.scrollY < pageHeight && navbar.classList.contains('nav-page-other')) {
    navbar.classList.remove('nav-page-other');
    navbar.classList.add('nav-page-1');
    navItems[0].classList.add('nav-item-active-1');

  } else if (this.scrollY >= pageHeight && navbar.classList.contains('nav-page-1')) {
